...
function getElementClass(character) {
    const charTechniques = (character.id === 'katara' || character.id === 'pakku')
        ? (character.techniquesFull || character.techniques)
        : character.techniques;

    if (!charTechniques || charTechniques.length === 0) {
...
function populateCharacterGrids() {
    ...
    const availableCharacters = Object.values(characters).filter(c => {
        if (c.id === 'katara' || c.id === 'pakku') {
            // For Katara and Pakku, check for techniquesFull or techniquesCanteen (the fix!)
            return (c.techniquesFull && c.techniquesFull.length > 0) || (c.techniquesCanteen && c.techniquesCanteen.length > 0);
        }
        // Others: require .techniques populated
        return c.techniques && c.techniques.length > 0;
    });
