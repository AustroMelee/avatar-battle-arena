...
import { generateStatusChangeEvent, generateCollateralDamageEvent } from './engine_narrative-engine.js';
...
export function applyEffect(effect, actor, target, battleState, battleEventLog) {
    ...
    let generatedEvents = []; // NEW: Array to collect events generated by this function

    // Store old values for status change logging
    let oldHp, oldEnergy, oldMentalState, oldMomentum, oldStunDuration;
    ...
    // Capture old values before applying effect
    if (primaryTarget) {
    ...
    switch (effect.type) {
        case EFFECT_TYPES.DAMAGE:
            if (primaryTarget && typeof primaryTarget.hp === 'number') {
                ...
                const hpEvent = generateStatusChangeEvent(battleState, primaryTarget, 'hp_change', oldHp, primaryTarget.hp, 'hp');
                if (hpEvent) generatedEvents.push(hpEvent);
            }
            break;
        ...
        case EFFECT_TYPES.ENERGY_CHANGE:
            if (primaryTarget && typeof primaryTarget.energy === 'number') {
                ...
                const energyEvent = generateStatusChangeEvent(battleState, primaryTarget, 'energy_change', oldEnergy, primaryTarget.energy, 'energy');
                if (energyEvent) generatedEvents.push(energyEvent);
            }
            break;
        case EFFECT_TYPES.STUN:
            if (primaryTarget) {
                ...
                const stunEvent = generateStatusChangeEvent(battleState, primaryTarget, 'stun_status_change', oldStunDuration, primaryTarget.stunDuration, 'stunDuration');
                if (stunEvent) generatedEvents.push(stunEvent);
            }
            break;
        case EFFECT_TYPES.MOMENTUM_CHANGE:
            if (effect.targetId) {
                ...
                if (targetCharacter) {
                    ...
                    const momentumEvent = generateStatusChangeEvent(battleState, targetCharacter, 'momentum_change', oldMomentum, targetCharacter.momentum, 'momentum');
                    if (momentumEvent) generatedEvents.push(momentumEvent);
                } else {
        ...
        case EFFECT_TYPES.CONDITIONAL_KO_OR_SELF_SABOTAGE:
            if (primaryTarget && actor) {
                ...
                generatedEvents.push(generateLogEvent(battleState, {
                    type: "dice_roll",
        ...
        case EFFECT_TYPES.TRIGGER_NARRATIVE_EVENT:
            ...
            generatedEvents.push({ type: 'narrative_event', eventId: effect.eventId, text: effect.message || "" });
        ...
        case EFFECT_TYPES.NARRATIVE_ONLY:
            ...
            generatedEvents.push(generateLogEvent(battleState, { type: 'narrative_event', eventId: effect.eventId, text: effect.message || "" }));
            break;
        default:
    ...
    return { success, message, events: generatedEvents };
} 